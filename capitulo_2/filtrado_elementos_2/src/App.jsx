...
import NotasImportantes from './components/NoteImpo'
...
const App = (props) => {
  ...
  */
  ...
  const importantes=notes.filter(nota=>nota.important==true)
  //agregar un boton que muestre los elementos importantes y otro los no importantes
  ...
  return (
    <div>
      ...
      <ul>
       {importantes.map((nota)=>
        <NotasImportantes key={nota.id} lista={nota}/>)}
      </ul>
    </div>
  )
}
...
const Boton=({funcion})=>{
  return(
    <div>
      <button onClick={funcion}>Importantes</button>
    </div>
  )
}
